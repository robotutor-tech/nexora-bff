...
const validateConditionConfig = (data: z.infer<typeof CreateConditionSchema>['config']) => {
  switch (data.type) {
    case 'FEED':
      return FeedConditionConfigSchema.safeParse(data)
    case 'TIME_RANGE':
      return TimeRangeConditionConfigSchema.safeParse(data)
    default:
      return null
  }
}

const FeedConditionConfigSchema = z
  .object({
    type: z.enum(['FEED']),
...
const TimeRangeConditionConfigSchema = z
  .object({
...
export const CreateConditionSchema = z
  ...
  .superRefine((data, ctx) => {
    const result = validateConditionConfig(data.config)
    if (!result) {
