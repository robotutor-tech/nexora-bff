import { UpdateTokenSchema } from './updateToken.schema'

describe('UpdateTokenSchema (bff-iot)', () => {
  const valid = { actorId: 'a-1', roleId: 'r-1' }

  it('should pass for a valid payload', () => {
    const result = UpdateTokenSchema.safeParse(valid)
  ...
  it('should fail when actorId is not a string', () => {
    ...
    const result = UpdateTokenSchema.safeParse(data)
